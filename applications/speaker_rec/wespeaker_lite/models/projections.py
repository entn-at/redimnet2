...
def get_projection(conf):
    if conf['project_type'] == 'add_margin':
        ...
        projection = SphereFace2(conf['embed_dim'],
                                 ...
                                 dynamic_margin=conf.get('dynamic_margin', True))
    else:
...
class SphereFace2(nn.Module):
    ...
    def forward(self, input, label, target_mask=None, reduction='mean'):
        # compute similarity
        ...
        # `target_mask` may be a soft label produced by mixup/cutmix: a convex
        # combination lambda*one_hot(a) + (1-lambda)*one_hot(b) of the present
        # speakers (it sums to 1 per row). For a single clean sample it is one-hot.
        if self.dynamic_margin:
            # --- Strict k-hot + lambda-margin + presence weighting ---
            # SphereFace2 is N independent one-vs-all binary heads, so a mixed clip
            # genuinely "contains" both speakers: give each present class a POSITIVE
            # head (no negative push), and let only the truly-absent classes be
            # negatives. The mix weight (presence) enters as BOTH:
            #   * the positive margin   -> a faintly-present speaker gets an easy bar
            #     (margin lambda*m), so its gradient fades smoothly instead of being
            #     forced over the full margin or contradicted by a negative term;
            #   * the loss weight        -> via target_mask in the loss sum below.
            # The negative heads keep the full margin m.
            # For a one-hot target_mask this reduces *exactly* to vanilla SphereFace2.
            khot = (target_mask > 0).to(target_mask.dtype)
            # rows with all-zero target_mask (e.g. sub-threshold views from
            # OverlapMixVad) must contribute zero loss — including nontarget
            ...
            # Original SphereFace2: soft k-hot, constant margin (back-compat path).
        ...
        if self.margin_type == 'A':  # arcface type
            ...
            if self.dynamic_margin:
                ...
                cos_m_theta_p = self.scale * self.fun_g(
                    torch.where(cos > th_p, cos * cos_m_p - sin * sin_m_p,
                                cos - mmm_p), self.t) + self.bias[0][0]
            else:
                cos_m_theta_p = self.scale * self.fun_g(
                    torch.where(cos > self.th, cos * self.cos_m - sin * self.sin_m,
                                cos - self.mmm), self.t) + self.bias[0][0]
            # negative head: full margin m on the genuinely-absent classes
        ...
        # logits for the accuracy meter (argmax vs the hard label)
        cos1 = (cos - margin_p) * (1.0 - nontarget_mask) + cos * nontarget_mask
        output = self.scale * cos1
        # present classes -> weighted positive BCE; absent classes -> negative BCE
        loss_per_sample = (target_mask * cos_p_theta +
                           nontarget_mask * cos_n_theta).sum(1)  # [B]
        if reduction == 'none':
            return output, loss_per_sample
        return output, loss_per_sample.mean()

    def extra_repr(self):
        return '''in_features={}, out_features={}, scale={}, lanbuda={},
                  margin={}, t={}, margin_type={}, dynamic_margin={}'''.format(
            self.in_features, self.out_features, self.scale, self.lanbuda,
            self.margin, self.t, self.margin_type, self.dynamic_margin)
...
class ArcMarginProduct(nn.Module):
    ...
    def forward(self, input, label, one_hot=None):
