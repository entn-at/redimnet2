...
class ExponentialDecayPlateauWithWarmup(BaseClass):
    def __init__(self,
                 optimizer,
                 num_epochs,
                 epoch_iter,
                 ...
                 staircase = True,
                 warm_from_zero = False):
        super().__init__(optimizer, num_epochs, epoch_iter, initial_lr, final_lr, warm_up_epoch, scale_ratio, warm_from_zero)
        self.warmup_steps = warm_up_epoch * epoch_iter
        self.plateau_steps = plateau_epochs * epoch_iter
        self.decay_steps = decay_epochs * epoch_iter
    ...
    def get_current_lr(self):
        ...
        if step < self.warmup_steps + self.plateau_steps:
            ...
            decay_steps_adjusted = step - (self.warmup_steps + self.plateau_steps)
            if self.staircase:
                current_lr = self.initial_lr * (self.decay_rate ** (decay_steps_adjusted // self.decay_steps))
            else:
                current_lr = self.initial_lr * math.exp((decay_steps_adjusted / self.decay_steps) * math.log(self.decay_rate))

        return current_lr * lr_coeff
